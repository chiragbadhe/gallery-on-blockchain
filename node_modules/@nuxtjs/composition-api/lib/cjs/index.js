...
const shallowSsrRef = (value, key) => {
    validateKey(key);
    if (process.client)
        return CompositionApi.shallowRef(ssrValue(value, key));
...
function assign(target, source) {
    Object.entries(source).forEach(([key, value]) => {
        Vue__default['default'].set(target, key, value);
    });
    return target;
}
...
const getHeadOptions = (options) => {
    ...
    const head = options.head instanceof Function
        ? () => defu__default['default'](_head, options.head())
        : () => _head;
    return { _head, head };
};
/**
...
const useMeta = (init) => {
    ...
    refs.titleTemplate = CompositionApi.customRef((track, trigger) => {
        return {
            get() {
                track();
            ...
            set(newValue) {
                if (!_head.titleTemplate) {
                    Vue__default['default'].set(_head, 'titleTemplate', newValue);
                }
                else {
...
const loadFullStatic = (vm) => {
    ...
    for (const key in data) {
        Vue__default['default'].set(vm.$data, key, data[key]);
    }
};
    ...
    CompositionApi.onBeforeMount(() => {
        ...
        for (const key in data) {
            try {
                ...
                Vue__default['default'].set(vm, key, data[key]);
            }
            catch (e) {
...
const compositionApiModule = function () {
    ...
    const { dst: entryDst } = this.addTemplate({
        ...
        options: {
            ...
            publicPath: normalize__default['default'](path.join(((_c = this.options.router) === null || _c === void 0 ? void 0 : _c.base) || '', '/')),
            globalContext,
            globalNuxt,
        },
    });
